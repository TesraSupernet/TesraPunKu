...
import (
	"errors"
	"fmt"
	"github.com/DOSNetwork/core/edge_cm/service"
...
var (
	Version                string
	Build                  string
	SubscriptionLoadConfig []conf.LoadConf
)
...
func init() {
	...
	go func() {
		...
		var data configuration.ProofData

		data.ProofBytes = (*zk).Proofs
		data.PublicInputs = (*zk).PublicInputs
		data.TrustedSetupBytes = (*zk).TrustedSetup
		data.CompileJson = (*zk).CompileJson
		ztools.GlobalProofQueue.Push(data)
	}()
...
func startEdgeCm(c *cli.Context) error {
	configPath := fmt.Sprint(c.String("config"))
	if len(configPath) == 0 {
		fmt.Println("Load default edge cm config")
	} else {
		...
		if err != nil {
			...
			yCode := yaml.NewDecoder(f)
			err := yCode.Decode(&conf.Conf)
			if err != nil {
				fmt.Printf("decode %s error.\n", configPath)
				os.Exit(2)
			}
			clientVersionTrim := fmt.Sprint(c.String("p2pClientVersion"))
			if len(clientVersionTrim) != 0 {
				conf.Conf.P2pClient.DefaultVersion = clientVersionTrim
			}
		}
	}

	for _, loadConf := range SubscriptionLoadConfig {
		loadConf.LoadConf(&conf.Conf)
	}
...
func actionStart(c *cli.Context) (err error) {
	err = startEdgeCm(c)
	if err != nil {
		fmt.Println("Read edge cm config error : ", err)
		return
	}
	...
	configuration.Account = key.Address.String()
	configuration.Account = strings.ToLower(configuration.Account)
	//Read Configuration
...
func main() {
	...
	go func() {
		for true {
			...
			data.TrustedSetupBytes = (*zk).TrustedSetup
			data.CompileJson = (*zk).CompileJson
			tools.Logger.Debug("power=" + string(data.PublicInputs))
			tools.Logger.Debug("cuda success")
			ztools.GlobalProofQueue.Push(data)
			if ztools.GlobalProofQueue.Len() >= 5 {
				ztools.GlobalProofQueue.Pop()
			}
	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Action: actionStart,
			Flags: []cli.Flag{
				cli.StringFlag{Name: "config,c", Usage: "path of edge cm config file."},
				cli.StringFlag{Name: "p2pClientVersion,pv", Usage: "set p2p client version."},
			},
		},
		{
			Name:   "stop",
