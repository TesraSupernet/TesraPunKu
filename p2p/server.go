...
func (n *server) Request(ctx context.Context, id []byte, msg proto.Message) (p2pmsg P2PMessage, err error) {
	...
	defer opCancel()
	fmt.Println("begin start request....", time.Now())
	...
	if result, err = req.waitForResult(); err != nil {
		fmt.Println("wait for result is err", time.Now())
		//fmt.Println("waitForResult:"+req.msg.String())
		msgType := reflect.TypeOf(req.msg).String()
		fmt.Println("Request type:" + msgType + ",Request waitForResult failed:" + req.msg.String())
		err = &P2PError{err: errors.Errorf("Request waitForResult log : %w", err), dest: addr, t: time.Now()}
		n.logger.Error(err)
		return
	}
	fmt.Println("request ending....", time.Now())
