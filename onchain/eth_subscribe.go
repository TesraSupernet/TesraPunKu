...
var proxyTable = []func(ctx context.Context, proxy *dosproxy.DosproxySession) (chan interface{}, chan error){
	...
	SubscribeUploadPower: func(ctx context.Context, proxy *dosproxy.DosproxySession) (chan interface{}, chan error) {
		out := make(chan interface{})
		errc := make(chan error)
		opt := &bind.WatchOpts{}
		go func() {
			transitChan := make(chan *dosproxy.DosproxyUploadPower)
			defer close(transitChan)
			defer close(errc)
			defer close(out)

			sub, err := proxy.Contract.WatchUploadPower(opt, transitChan)
			if err != nil {
				replyError(ctx, errc, &OnchainError{err: errors.Errorf("SubscribeEvent err: %w", err), Idx: getIndex(ctx)})
				return
			}
