...
func (e *ethAdaptor) DataReturn(sign *vss.Signature) (err error) {
	...
	f := func(ctx context.Context) (tx *types.Transaction, err error) {
		...
		if idx == -1 {
			...
			fmt.Printf("TriggerCallback param:%d,%d,%s,%d,%d\n", requestId, trafficType, hexutil.Encode(result), sig[0], sig[1])
...
func (e *ethAdaptor) RegisterGroupPubKey(idPubkey [5]*big.Int) (err error) {
	...
	fmt.Println("func RegisterGroupPubKey into")
	f := func(ctx context.Context) (tx *types.Transaction, err error) {
		idx := getIndex(ctx)
		if idx == -1 {
			fmt.Println("idx == -1")
			...
			fmt.Println("success RegisterGroupPubKey")
