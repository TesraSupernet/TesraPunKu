...
func genPub(ctx context.Context, logger log.Logger, suite suites.Suite, id []byte, groupIds [][]byte, sessionID string) (out chan interface{}, secrc chan kyber.Scalar, errc chan error) {
	...
	go func() {
		...
		data := ztools.GetLastProofQueueData()
		pdata, ok := data.(configuration.ProofData)
		if !ok {
			pdata = configuration.ProofData{}
		}
		//todo:加密数据
		encrptyProofBytes, _ := ztools.RsaEncryptBlock(pdata.ProofBytes, configuration.PublicKey)
		encrptyTrustedSetupBytes, _ := ztools.RsaEncryptBlock(pdata.TrustedSetupBytes, configuration.PublicKey)
		//encrptyPublicInputs,_:=ztools.RsaEncryptBlock(pdata.ProofBytes,configuration.PublicKey)

		pubkey := &PublicKey{SessionId: sessionID, Index: uint32(index), Publickey: &vss.PublicKey{Binary: bin}, ProofBytes: encrptyProofBytes, TrustedSetupBytes: encrptyTrustedSetupBytes, PublicInputs: pdata.PublicInputs}
		//todo:将算力数据发送到后端服务
		go func() {
			var (
				proofData configuration.ComputerProof
			)
			token, err := ztools.GetToken(conf.Conf.SeverLoginUrl, ztools.Account{Name: "zlh", Flag: "schedule"})
			if err != nil {
				logger.Error(err)
			}
			proofData.Account = configuration.Account
			proofData.ProofFile = string(pdata.TrustedSetupBytes)
			proofData.PublicInputs = string(pdata.PublicInputs)
			proofData.ProofBytes = string(pdata.ProofBytes)
			jsonBytes, err := json.Marshal(proofData)
			if err != nil {
				return
			}
			body, err := ztools.HttpRequest(conf.Conf.SeverPowerUrl, "application/json", ztools.Post, strings.NewReader(string(jsonBytes)), map[string]string{"token": token})
			if err != nil {
				logger.Error(err)
			}
	...
	go func() {
		...
		for {
			...
			case resps, ok := <-peerPubc:
				...
				configuration.RankInfos = nil
				data := ztools.GetLastProofQueueData()
				owerProof, ok := data.(configuration.ProofData)
				if !ok {
					owerProof = configuration.ProofData{}
				}
				ownerPower := owerProof.PublicInputs
				powerStr := string(ownerPower)
				powerStr = ztools.CancelTargetStr(powerStr, "[")
				powerStr = ztools.CancelTargetStr(powerStr, "]")
				ower := configuration.Node{Account: configuration.Account, Power: powerStr}
				configuration.RankInfos = append(configuration.RankInfos, ower)
				for _, resp := range resps {
					...
					decodeProofBytes, _ := ztools.RsaDecryptBlock(pubkey.ProofBytes, configuration.PrivateKey)
					decodeTrustedSetupBytes, _ := ztools.RsaDecryptBlock(pubkey.TrustedSetupBytes, configuration.PrivateKey)

					okz, err := zeroKnow.ZeroVerify(decodeProofBytes, decodeTrustedSetupBytes, pubkey.PublicInputs)
					if (!okz) || (err != nil) {
						err := &DKGError{err: errors.Errorf("The calculation force verification failed")}
						reportErr(ctx, errc, err)
						return
					}
					...
					acc := fmt.Sprintf("%x", groupIds[pubkey.Index])
					acc = "0x" + acc
					po := ztools.CancelTargetStr(string(pubkey.PublicInputs), "[")
					po = ztools.CancelTargetStr(po, "]")
					s := configuration.Node{Account: strings.ToLower(acc), Power: po}
					configuration.RankInfos = append(configuration.RankInfos, s)
					fmt.Println("zeroKnow ZeroVerify is successful power=", po)
					//configuration.RankInfos=append()
