		...
		for {
			...
			default:
				if d.isGuardian {
					...
					d.logger.Debug("handled_0=" + strconv.FormatBool(handled))
					if !handled {
						handled = d.handleGroupFormation()
						d.logger.Debug("handled_handleGroupFormation=" + strconv.FormatBool(handled))
					}
					if !handled {
						handled = d.handleGroupDissolve(currentBlockNumber)
						d.logger.Debug("handled_handleGroupDissolve=" + strconv.FormatBool(handled))
					}
					if !handled {
						handled = d.handleBootstrap(currentBlockNumber)
						d.logger.Debug("handled_handleBootstrap=" + strconv.FormatBool(handled))
					}
				}
