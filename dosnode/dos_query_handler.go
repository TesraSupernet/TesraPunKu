...
func (d *DosNode) queryLoop() {
	...
	for {
		...
		case msg, ok := <-peerMsg:
			if ok {
				d.logger.Debug("receive:" + msg.Msg.String())
				err := d.p.Reply(context.Background(), msg.Sender, msg.RequestNonce, msg.Msg.Message)
				if err != nil {
					fmt.Printf("signature reply fail!\n")
				}
...
func (d *DosNode) handleQuery(ids [][]byte, pubPoly *share.PubPoly, sec *share.PriShare, groupID string, requestID, lastRand, useSeed *big.Int, url, selector string, pType uint32) {
	queryCtx, cancel := context.WithTimeout(context.Background(), 60*15*time.Second)
	start := time.Now()
	d.logger.Debug(time.Now().String())
	defer cancel()
	defer func() {
		temp := time.Now().Unix() - start.Unix()
		d.logger.Debug(time.Now().String() + "   queryCtx is cancel timestamp=" + strconv.FormatInt(temp, 10))
	}()
	...
	case onchain.TrafficUserQuery:
		...
		nHash := sha256.Sum256(bytes)
		nonce = nHash[:]
	case onchain.TrafficUploadPower:
		fmt.Println("case onchain.TrafficUploadPower:")
		var bytes []byte
		bytes = append(bytes, []byte(groupID)...)
		bytes = append(bytes, requestID.Bytes()...)
		bytes = append(bytes, lastRand.Bytes()...)
	...
	case onchain.TrafficUserQuery:
		...
		errcList = append(errcList, errc)
	case onchain.TrafficUploadPower:
		contentc, errc = genUploadPower(queryCtxWithValue, submitterc[0], d.logger)
		errcList = append(errcList, errc)
	}
